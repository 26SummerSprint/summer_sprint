...
| `requirements-client.txt` | ✅ | 팀원 로컬에서 조회만 할 때 필요한 최소 의존성 (requests) | 팀원용 경량 실행 환경 구성 | `pip install -r requirements-client.txt` |
...
```bash
pip install -r requirements-client.txt   # requests만 설치
...
- 수집·임베딩 파이프라인(`main.py --backfill`/`--daily`)은 EC2 인스턴스 한 곳에서만 돌리세요. 여러 곳에서 동시 실행할 필요가 없는 구조입니다(팀원은 API로 조회만 함).
